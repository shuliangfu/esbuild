 ...
 */
function findVueRuntimePath(moduleCache: ModuleCache): string | undefined {
  for (const [specifier, localPath] of moduleCache.entries()) {
    if (!/^npm:vue@/.test(specifier) || !localPath) continue;
 ...
 *
 * @param moduleCache 预构建的模块缓存（需在 denoResolverPlugin 之前通过 buildModuleCache 得到）
    ...
    setup(build) {
      build.onResolve(
        ...
        (): esbuild.OnResolveResult | undefined => {
          const runtimePath = findVueRuntimePath(moduleCache);
