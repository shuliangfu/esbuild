...
/**
 * 将 JSR 协议路径拆成「包身份」与「包内路径」，避免相对路径 ../ 回退时误删包名。
 * 例如：jsr:@dreamer/websocket@^1.0.1/client -> { packageBase: "jsr:@dreamer/websocket@^1.0.1", pathWithinPackage: "client" }
 * 这样 ../encryption.ts 只需在 pathWithinPackage 上回退，得到 encryption.ts，不会变成 jsr:@dreamer/encryption。
 *
 * @param protocolPath - 以 jsr: 开头的完整协议路径
 * @returns 拆开后的包身份与包内路径，非 jsr 或格式不符时返回 null
 */
function parseJsrPackageBaseAndPath(
  ...
  // 匹配 @scope/name@version（version 可含 ^ ~ 和 -），后面可跟 /path
  const match = after.match(/^(@[^/]+\/[^/]+\@[^/]+)(?:\/(.*))?$/);
  if (!match) return null;
  const packageBase = "jsr:" + match[1];
  const pathWithinPackage = match[2] ?? "";
  return { packageBase, pathWithinPackage };
}
  ...
  return {
    ...
    setup(build) {
      ...
      function getLocalPathFromCache(specifier: string): string | undefined {
        ...
        // 缓存 key 不含 ^、~，先统一去掉再匹配
        if (specifier.startsWith("jsr:")) {
          const specifierForCache = specifier.replace(/@([\^~])(\d)/g, "@$2");
          const afterJsr = specifierForCache.slice(4);
          const lastAtIdx = afterJsr.lastIndexOf("@");
          const versionAndPathRaw = afterJsr.slice(lastAtIdx + 1);
          const versionPart = versionAndPathRaw.replace(/\/.*$/, "").trim();
          ...
          if (hasVersion) {
            ...
            if (subpath) {
              ...
              // 带 ^/~ 时 strip 后的精确版本可能不在缓存（如 ^1.0.8 解析为 1.0.9），
              // 按 scope+name+subpath 在缓存中找任意版本，避免有缓存仍走网络
              const prefixRange = `jsr:${scopeAndName}@`;
              // 规范化后比较：包内路径去 src/ 和扩展名，与 export subpath 做通用匹配，不写死 mod、adapters 等
              // 首段+末段匹配时要求 normPkg 段数不少于 normSub，避免 client/components/mod（3 段）错误命中 client/mod（2 段）导致死循环
              const normSub = subpath.replace(/\.(tsx?|jsx?)$/i, "");
              const normSubSegs = normSub ? normSub.split("/").length : 0;
              const firstSub = normSub ? normSub.split("/")[0] : "";
              const lastSub = normSub ? normSub.split("/").pop() ?? "" : "";
              for (const [k, v] of moduleCache.entries()) {
                if (!k.startsWith(prefixRange) || !existsSync(v)) continue;
                const afterVer = k.slice(prefixRange.length);
                const pathInPkg = afterVer.includes("/")
                  ? afterVer.slice(afterVer.indexOf("/") + 1)
                  : "";
                const normPkg = pathInPkg
                  .replace(/^src\//, "")
                  .replace(/\.(tsx?|jsx?)$/i, "");
                const normPkgSegs = normPkg ? normPkg.split("/").length : 0;
                const matchSub = normPkg === normSub ||
                  normPkg === normSub + "/mod" ||
                  normPkg.replace(/\/mod$/, "") === normSub ||
                  (firstSub &&
                    lastSub &&
                    normPkgSegs >= normSubSegs &&
                    normPkg.startsWith(firstSub + "/") &&
                    normPkg.endsWith("/" + lastSub));
                if (matchSub) {
                  resolverCacheKeyForLoader.set(specifier, k);
                  debugLog(
                    `getLocalPathFromCache 预构建缓存范围命中(任意版本): ${specifier} -> ${k} -> ${v}`,
                  );
                  return v;
                }
              }
            ...
            const direct = moduleCache.get(specifierForCache);
        ...
        async (args): Promise<esbuild.OnResolveResult | undefined> => {
          ...
          try {
              ...
              try {
                ...
                if (out.success && out.stdout && out.stdout.length > 0) {
                  ...
                  if (line.startsWith("file://")) {
                    importerUrl = line;
                    debugLog(
                      `onResolve 相对路径：子进程用项目 config 解析 importer 得到 fileUrl=${importerUrl}`,
                    );
                  }
                }
              } catch {
            ...
            try {
              ...
              let fullProtocolPath: string;

              if (importerProtocolPath.startsWith("jsr:")) {
                const parsed = parseJsrPackageBaseAndPath(importerProtocolPath);
                if (parsed) {
                  // 仅在「包内路径」上解析 ../ 和 ./, 不修改包身份，避免 ../encryption.ts 变成 jsr:@dreamer/encryption
                  // base 用 importer 所在目录：预构建缓存命中则 importer 必为单文件，用 pathWithinPackage 的父目录；否则按扩展名、/mod 推断
                  let basePath = (parsed.pathWithinPackage || "").replace(
                    /\/$/,
                    "",
                  );
                  const importerCached = !!(
                    moduleCache && getLocalPathFromCache(importerProtocolPath)
                  );
                  if (importerCached) {
                    // 缓存命中说明 importer 对应单文件，相对路径的 base 为该文件所在目录（pathWithinPackage 去掉最后一段）
                    basePath = basePath.replace(/\/[^/]+$/, "") || basePath;
                  } else if (/\.(tsx?|jsx?|mts|mjs)$/i.test(basePath)) {
                  ...
                  const pathForProtocol = /\.(tsx?|jsx?)$/i.test(resolvedWithin)
                    ...
                    : resolvedWithin;
                  const baseForProtocol = parsed.packageBase.replace(
                    /@([\^~])([^/]+)/,
                    "@$2",
                  );
                  fullProtocolPath = pathForProtocol
                    ? `${baseForProtocol}/${pathForProtocol}`
                    : baseForProtocol;
                } else {
                  const baseForProtocol = importerProtocolPath.replace(
                    /@([\^~])([^/]+)/,
                    "@$2",
                  );
                  fullProtocolPath = `${baseForProtocol}/${
                ...
                fullProtocolPath = `${currentBasePath}/${normalizedPath}`;
              }
