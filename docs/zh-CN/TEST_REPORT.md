...
- **执行时间**：约 34s（Deno `deno test -A`），约 4s（Bun `bun test`）
...
| `builder-bundle.test.ts`            | 29     | ✅ 全部通过 | 简单打包器功能                                                                                          |
...
| `builder-client.test.ts`            | 31     | ✅ 全部通过 | 客户端构建器功能（含 preact/react 多引擎）                                                              |
...
| `server-module-detector.test.ts`    | 25     | ✅ 全部通过 | 服务端模块检测插件                                                                                      |
| `resolver-advanced.test.ts`         | 17     | ✅ 全部通过 | 解析器插件高级测试（含 view/TSX/JSX 编译；生成文件位于 tests/data/resolver-advanced，测试结束自动清理） |
...
- **解析器**：Deno/Bun 解析器、JSR/npm 子路径、路径别名、相对路径、view
  主入口相对路径与 .tsx loader 编译；解析器高级测试生成文件统一放在
  tests/data/resolver-advanced，套件结束后自动删除
...
- **特性**：多引擎（preact/react）、write/mode、external、useNativeCompile、路径解析（deno.json、tsconfig）
...
- **569** 个用例（Deno `deno test -A`，全部通过，约 34s）
