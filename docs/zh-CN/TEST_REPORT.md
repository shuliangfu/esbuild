...
- **执行时间**：约 25s（Deno `deno test -A`），约 4s（Bun `bun test`）
...
| `resolver-advanced.test.ts`         | 16     | ✅ 全部通过 | 解析器插件高级测试（含 view/TSX 编译；生成文件位于 tests/data/resolver-advanced，测试结束自动清理） |
...
- **568** 个用例（Deno `deno test -A`，全部通过，约 25s）
